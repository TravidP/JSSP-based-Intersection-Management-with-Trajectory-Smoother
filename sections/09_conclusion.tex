...
% CN-SECTION: 结论总结层次化调度和平滑执行框架，并强调在线部署只做策略rollout。
...
\begin{addedtwoblock}
The second-round formulation further specifies the operation-graph representation and a relation-aware multi-head attention policy. This addition gives the future implementation a concrete interface: node features describe operation status, vehicle motion, delay, route progress, and masks, while typed graph edges distinguish same-vehicle precedence from same-zone conflict.
\end{addedtwoblock}
