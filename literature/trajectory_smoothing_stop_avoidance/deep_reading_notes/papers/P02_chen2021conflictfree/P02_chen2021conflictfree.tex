...
\begin{itemize}
\item 能否独立重写核心公式：冲突图与排序目标 / Conflict graph and sequencing objective，并解释每个符号的物理意义？
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P02_chen2021conflictfree/01_title_and_abstract_p1.jpg}
\caption{首页与摘要 / Title and abstract page (p.1, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P02_chen2021conflictfree/02_modeling_or_problem_setup_p2.jpg}
\caption{建模或问题设定页 / Modeling or problem-setup page (p.2, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P02_chen2021conflictfree/03_core_method_or_algorithm_p4.jpg}
\caption{核心方法或算法页 / Core method or algorithm page (p.4, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P02_chen2021conflictfree/04_experiments_or_results_p11.jpg}
\caption{实验或结果页 / Experiments or results page (p.11, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P02_chen2021conflictfree/05_conclusion_or_limitations_p17.jpg}
\caption{结论或局限页 / Conclusion or limitations page (p.17, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript.}
\end{figure}
...
\textbf{关键论点 / Key argument：} 这一部分通常把痛点落到 Graph conflict modeling + passing order：既要提高效率，又要保持安全与在线可执行。
...
\subsection*{p.2 II. PROBLEM STATEMENT}
...
\textbf{关键论点 / Key argument：} 本节是复现论文的核心入口，应与公式“冲突图与排序目标 / Conflict graph and sequencing objective”一起读。
...
\subsection*{p.4 III. METHODOLOGY}
\textbf{章节目标 / Learning goal：} 解释算法如何把模型转化为可执行决策，并说明在线运行机制。

Trace how the paper turns the model into executable online decisions.

\textbf{关键论点 / Key argument：} 先依据几何轨迹和时间可达性构建冲突有向图与共存无向图；再用改进深度优先生成树和最小团覆盖等图算法求通行组与顺序；最后由分布式控制保证每辆车按序到达冲突区。
...
\textbf{关键论点 / Key argument：} 实验应与论文声称的贡献闭环：Traffic simulations over varying vehicle counts and volumes; reports efficiency and fuel-economy improvements.

\textbf{技术焦点 / Technical focus：} 重点追踪指标：平均延误 (average delay)、总清空时间 (evacuation time)、吞吐量 (throughput)、燃油/能耗代理和停车次数。 同时检查仿真平台、交通需求和随机性设置。

\textbf{审稿追问 / Reviewer question：} 实验是否只展示平均收益，还是覆盖了高密度、异常扰动和失败案例？

\subsection*{p.17 V. CONCLUSION}
...
\paragraph{Opening motivation / 开篇动机段}先把作者的痛点翻译成一句博士问题：如何把无信号交叉口中复杂的相交、跟驰、合流和分流关系转化为可求解的图排序问题？ 读这一段时不要急着接受动机，要追问传统 FCFS、信号灯或保守 MPC 为什么不足。

Turn the opening motivation into one doctoral-level question and test whether the paper truly needs a new method.

\paragraph{Assumption and setting / 假设与场景段}把车辆类型、通信条件、路口类型和动力学假设列成表。该文的关键边界包括：车辆轨迹模板和冲突点可提前计算。；车辆遵守分配的通行顺序，通信近似理想。；主要聚焦纵向控制，横向变道不是核心问题。

List vehicle type, communication, intersection setting, and dynamics assumptions before reading the equations.

\paragraph{Model and notation / 模型与符号段}围绕核心公式“冲突图与排序目标 / Conflict graph and sequencing objective”复写符号表，确认每个变量是决策变量、状态变量、参数还是约束阈值。
...
\paragraph{Algorithm mechanism / 算法机制段}把算法读成一条数据流：输入交通状态，执行 Models trajectory conflicts as a conflict directed graph and coexisting undirected graph; solves passing order through improved depth-first spanning tree and minimum clique cover methods.，输出可执行通行/控制决策，并检查安全接口。
...
\paragraph{Experiment and evidence / 实验证据段}不要只看作者报告的提升，要检查基准、公平性和指标。本文验证描述为：Traffic simulations over varying vehicle counts and volumes; reports efficiency and fuel-economy improvements.

Go beyond reported gains and inspect baselines, fairness, metrics, and computation time.

\paragraph{Reviewer synthesis / 审稿式总结段}最后把贡献拆成可发表与需补强两部分。对你课题最直接的用途是：Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript.
...
2 & 建模 / Modeling & 冲突图与排序目标 / Conflict graph and sequencing objective \\
3 & 核心求解 / Core solver & Models trajectory conflicts as a conflict directed graph and coexisting undirected graph; solves passing order through improved depth-first spanning tree and minimum clique cover methods. \\
4 & 安全接口 / Safety interface & Conflict-free graph edges represent crossing, diverging, converging, and reachability conflicts before scheduling. \\
5 & 平滑与效率 / Smoothing and efficiency & Optimizes evacuation time and average delay, using distributed control to reduce idling near the stop line. \\
6 & 验证 / Validation & Traffic simulations over varying vehicle counts and volumes; reports efficiency and fuel-economy improvements. \\
\bottomrule
\end{longtable}
...
\midrule
理论贡献 / Theoretical contribution & 4/5 & 冲突图与排序目标 / Conflict graph and sequencing objective \\
算法贡献 / Algorithmic contribution & 5/5 & Models trajectory conflicts as a conflict directed graph and coexisting undirected graph; solves passing order through improved depth-first spanning tree and minimum clique cover methods. \\
实验贡献 / Experimental contribution & 3/5 & Traffic simulations over varying vehicle counts and volumes; reports efficiency and fuel-economy improvements. \\
工程贡献 / Engineering contribution & 3/5 & Conflict-free graph edges represent crossing, diverging, converging, and reachability conflicts before scheduling. \\
博士课题价值 / PhD relevance & 5/5 & Strong support for representing intersection conflicts as typed graph relations, similar to the operation graph in this manuscript. \\
\bottomrule
\end{longtable}
