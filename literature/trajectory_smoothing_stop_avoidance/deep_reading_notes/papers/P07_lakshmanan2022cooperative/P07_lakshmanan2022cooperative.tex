...
\begin{itemize}
\item 能否独立重写核心公式：PMP 速度规划问题 / PMP speed-profile optimal control，并解释每个符号的物理意义？
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P07_lakshmanan2022cooperative/01_title_and_abstract_p1.jpg}
\caption{首页与摘要 / Title and abstract page (p.1, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P07_lakshmanan2022cooperative/02_modeling_or_problem_setup_p2.jpg}
\caption{建模或问题设定页 / Modeling or problem-setup page (p.2, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P07_lakshmanan2022cooperative/03_core_method_or_algorithm_p4.jpg}
\caption{核心方法或算法页 / Core method or algorithm page (p.4, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P07_lakshmanan2022cooperative/04_experiments_or_results_p7.jpg}
\caption{实验或结果页 / Experiments or results page (p.7, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{../../assets/P07_lakshmanan2022cooperative/05_supplemental_reading_page_p8.jpg}
\caption{补充精读页 / Supplemental close-reading page (p.8, full\_page). 这张截图用于把笔记中的抽象概念拉回原论文页面，便于你平行阅读原文、公式、图表和实验设置。 与当前课题的连接：Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance.}
\end{figure}
...
\textbf{关键论点 / Key argument：} 这一部分通常把痛点落到 Eco-driving optimal speed profile：既要提高效率，又要保持安全与在线可执行。
...
\subsection*{p.2 2. INTERSECTION MODEL, VEHICLE MODEL AND}
...
\textbf{关键论点 / Key argument：} 本节是复现论文的核心入口，应与公式“PMP 速度规划问题 / PMP speed-profile optimal control”一起读。
...
\subsection*{p.2 2.1 Intersection Model}
\textbf{章节目标 / Learning goal：} 把自然语言交通问题压缩为变量、状态、约束、目标函数或图结构。
...
\subsection*{p.3 2.2 Vehicle Model}
\textbf{章节目标 / Learning goal：} 把自然语言交通问题压缩为变量、状态、约束、目标函数或图结构。
...
\subsection*{p.4 3. OPTIMAL CONTROL PROBLEM FORMULATION}
\textbf{章节目标 / Learning goal：} 把自然语言交通问题压缩为变量、状态、约束、目标函数或图结构。
...
\subsection*{p.4 3.1 Unconstrained Eco-Driving Problem}
\textbf{章节目标 / Learning goal：} 把自然语言交通问题压缩为变量、状态、约束、目标函数或图结构。
...
\subsection*{p.4 3.2 Car-Following conﬂict/Sequential Conﬂict}
...
\subsection*{p.5 3.3 Crossing Conﬂict}
\textbf{章节目标 / Learning goal：} 承接前后章节，补充局部定义、案例或实现细节。
...
\paragraph{Opening motivation / 开篇动机段}先把作者的痛点翻译成一句博士问题：在无信号交叉口协作中，如何用解析最优控制生成节能、少停顿的速度曲线？ 读这一段时不要急着接受动机，要追问传统 FCFS、信号灯或保守 MPC 为什么不足。

Turn the opening motivation into one doctoral-level question and test whether the paper truly needs a new method.

\paragraph{Assumption and setting / 假设与场景段}把车辆类型、通信条件、路口类型和动力学假设列成表。该文的关键边界包括：车辆动力学和能耗模型可精确获得。；冲突关系可以预先分类并转成边界/时间约束。；场景规模相对有限。

List vehicle type, communication, intersection setting, and dynamics assumptions before reading the equations.

\paragraph{Model and notation / 模型与符号段}围绕核心公式“PMP 速度规划问题 / PMP speed-profile optimal control”复写符号表，确认每个变量是决策变量、状态变量、参数还是约束阈值。
...
\paragraph{Algorithm mechanism / 算法机制段}把算法读成一条数据流：输入交通状态，执行 Single-level eco-driving optimization solved with Pontryagin's Minimum Principle for electric CAV speed profiles at an unsignalized intersection.，输出可执行通行/控制决策，并检查安全接口。
...
\paragraph{Experiment and evidence / 实验证据段}不要只看作者报告的提升，要检查基准、公平性和指标。本文验证描述为：Simulation comparison of cooperative and non-cooperative eco-driving against IDM baseline.

Go beyond reported gains and inspect baselines, fairness, metrics, and computation time.

\paragraph{Reviewer synthesis / 审稿式总结段}最后把贡献拆成可发表与需补强两部分。对你课题最直接的用途是：Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance.
...
2 & 建模 / Modeling & PMP 速度规划问题 / PMP speed-profile optimal control \\
3 & 核心求解 / Core solver & Single-level eco-driving optimization solved with Pontryagin's Minimum Principle for electric CAV speed profiles at an unsignalized intersection. \\
4 & 安全接口 / Safety interface & Analytical conflict cases and cooperation levels encode interaction safety among CAVs. \\
5 & 平滑与效率 / Smoothing and efficiency & Focuses directly on optimal speed profiles that reduce energy use and avoid inefficient stop-and-go behavior. \\
6 & 验证 / Validation & Simulation comparison of cooperative and non-cooperative eco-driving against IDM baseline. \\
\bottomrule
\end{longtable}
...
\midrule
理论贡献 / Theoretical contribution & 4/5 & PMP 速度规划问题 / PMP speed-profile optimal control \\
算法贡献 / Algorithmic contribution & 4/5 & Single-level eco-driving optimization solved with Pontryagin's Minimum Principle for electric CAV speed profiles at an unsignalized intersection. \\
实验贡献 / Experimental contribution & 3/5 & Simulation comparison of cooperative and non-cooperative eco-driving against IDM baseline. \\
工程贡献 / Engineering contribution & 3/5 & Analytical conflict cases and cooperation levels encode interaction safety among CAVs. \\
博士课题价值 / PhD relevance & 4/5 & Supports the smoothing objective terms for acceleration, energy proxy, and stop avoidance. \\
\bottomrule
\end{longtable}
...
\paragraph{p.4 3.2 Car-Following conﬂict/Sequential Conﬂict}作为局部论证段落阅读，关注它如何服务主问题。 / Read as a local argument and ask how it supports the main question.

\paragraph{p.5 3.3 Crossing Conﬂict}作为局部论证段落阅读，关注它如何服务主问题。 / Read as a local argument and ask how it supports the main question.
